...
import { Ticket, Message, TicketStatus, User, TicketPriority } from '../types';
import { fetchTicketById, sendTicketMessage, uploadAttachment, updateTicket, fetchSupportTeam } from '../services/api';
import AdminSidebar from '../components/AdminSidebar';
import DashboardHeader from '../components/DashboardHeader';
import { Loader, Send, Paperclip, ArrowLeft, FileText } from 'lucide-react';
...
const ChatBubble: React.FC<{ message: Message; isOwn: boolean }> = ({ message, isOwn }) => {
    const bubbleClasses = isOwn
        ? 'bg-cyan-600 text-white self-end rounded-l-lg rounded-br-lg'
...
const AdminTicketViewPage: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const loadData = async () => {
            try {
                const [ticketData, teamData] = await Promise.all([fetchTicketById(id), fetchSupportTeam()]);
        ...
        socketRef.current?.on('newMessage', (message: Message) => setMessages(prev => [...prev, message]));
    ...
    const handleTicketUpdate = async (data: { status?: TicketStatus; assignedTo?: string; priority?: TicketPriority }) => {
        if (!id) return;
        try {
    ...
    if (isLoading) return <div className="flex justify-center items-center h-screen"><Loader className="w-12 h-12 animate-spin text-cyan-700" /></div>;
        ...
        <div className="flex h-screen bg-gray-100 font-sans">
            ...
            <div className="relative flex-1 flex flex-col overflow-hidden lg:ml-64">
                ...
                <main className="flex-1 flex flex-col bg-slate-50 p-6 lg:p-8">
                    <div className="mb-4">
                        <Link to="/admin/support" className="inline-flex items-center text-sm font-semibold text-gray-600 hover:text-gray-800">
                    ...
                    <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6 overflow-hidden">
                        <div className="lg:col-span-2 bg-white rounded-xl shadow-sm flex flex-col h-full">
                            ...
                            <div className="p-4 border-t bg-gray-50">
                                <form onSubmit={handleSendMessage} className="flex items-center gap-3">
                                    ...
                                    <button type="button" onClick={() => fileInputRef.current?.click()} disabled={isSending} className="p-2 text-gray-500 hover:text-cyan-600 disabled:opacity-50">
                                        <Paperclip size={20} />
                                    </button>
                                    <input
                                        type="text"
                                        ...
                                        className="flex-1 p-2 border rounded-full px-4 focus:outline-none focus:ring-2 focus:ring-cyan-500"
                                        disabled={isSending || ticket.status === 'closed'}
                                    />
                                    <button type="submit" disabled={isSending || !newMessage.trim() || ticket.status === 'closed'} className="bg-cyan-600 text-white rounded-full p-2 hover:bg-cyan-700 disabled:bg-cyan-300">
                        ...
                        <div className="bg-white rounded-xl shadow-sm p-6 space-y-4 self-start">
                            ...
                            <div className="text-sm">
                                ...
                                <select value={ticket.status} onChange={e => handleTicketUpdate({ status: e.target.value as TicketStatus })} className="w-full p-2 border rounded-md">
                             ...
                             <div className="text-sm">
                                ...
                                <select value={ticket.priority} onChange={e => handleTicketUpdate({ priority: e.target.value as TicketPriority })} className="w-full p-2 border rounded-md">
                            ...
                            <div className="text-sm">
                                <label className="font-semibold text-gray-600 block mb-1">Assigned To</label>
                                <select value={ticket.assignedTo?._id || ''} onChange={e => handleTicketUpdate({ assignedTo: e.target.value })} className="w-full p-2 border rounded-md">
                                    <option value="">Unassigned</option>
                                    {supportTeam.map(member => <option key={member._id} value={member._id}>{member.name}</option>)}
                                </select>
                            </div>
                             <div className="text-sm">
