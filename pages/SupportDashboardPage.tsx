...
const SupportDashboardPage: React.FC = () => {
    ...
    const handleSelectChat = useCallback(async (chat: any) => {
        if (chat.status === 'waiting') {
            socketRef.current?.emit('adminJoinsChat', { sessionId: chat._id, adminUser: supportUser });
        } else {
    ...
    const counts = useMemo(() => ({
        ...
        assignedToMe: liveChats.filter(c => c.status === 'active' && c.admin?._id === supportUser?._id).length,
        agent: liveChats.filter(c => c.status === 'active').length,
        awaitingAgent: liveChats.filter(c => c.status === 'waiting').length,
        paused: 0,
    }), [liveChats, supportUser]);
    ...
    const filteredChats = useMemo(() => {
        ...
        switch (activeInboxFilter) {
            case 'Assigned to me':
                chats = chats.filter(chat => chat.admin?._id === supportUser?._id && chat.status === 'active');
                break;
            case 'Unassigned':
        ...
        return chats;
    }, [liveChats, activeInboxFilter, activeStatusFilter, supportUser]);

    const isReadOnly = useMemo(() => {
        if (!selectedSession || !supportUser) return true;
        if (!selectedSession.admin) return false;
        return selectedSession.admin._id !== supportUser._id;
    }, [selectedSession, supportUser]);
