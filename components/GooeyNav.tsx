...
const GooeyNav: React.FC<GooeyNavProps> = ({
  ...
  const createParticle = (i: number, t: number, d: number[], r: number) => {
    ...
    // FIX: Use nullish coalescing operator to provide fallback values, ensuring type safety when accessing array elements.
    const colorValue = colors[Math.floor(Math.random() * colors.length)] ?? 1;
    return {
      start: getXY(d[0] ?? 90, particleCount - i, particleCount),
      end: getXY((d[1] ?? 10) + noise(7), particleCount - i, particleCount),
      time: t,
      scale: 1 + noise(0.2),
      color: colorValue,
