...
const useResizeObserver = (callback: () => void, elements: React.RefObject<HTMLElement>[], dependencies: any[]) => {
  useEffect(() => {
    ...
    // Use feature detection to choose the implementation
    if ('ResizeObserver' in window) {
      ...
      // FIX: Cast `window` to `any` to resolve a type error where `window` is inferred as `never`, likely due to conflicting global type definitions.
...
export const LogoLoop: React.FC<LogoLoopProps> = memo(
  ({
    ...
    useAnimationLoop(trackRef, targetVelocity, seqWidth, isHovered, isHovered && pauseOnHover);
